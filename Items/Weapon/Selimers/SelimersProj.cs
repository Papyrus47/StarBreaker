...
namespace StarBreaker.Items.Weapon.Selimers
{
    public class SelimersProj : ModProjectile
    {
        ...
        {
            Texture2D texture = ModContent.Request<Texture2D>((GetType().Namespace + ".SelimersProj" + Projectile.frame.ToString()).Replace('.', '/'), ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
